...
# counting the number of different postal codes per municipality
areasDF= task2DF \
    .groupBy("municipality") \
    .agg(countDistinct("postal_code").alias("areas"))

# counting the number of different streets per municipality
streetsDF= task2DF \
    .groupBy("municipality") \
    .agg(countDistinct("street").alias("streets"))

# counting the number of different buildings per municipality
buildingsDF= task2DF \
    .groupBy("municipality") \
    .agg(countDistinct("building_id").alias("buildings"))

# calculating the minimum distance between the Kampusareena building and all other buildings
# selecting the minimum distance per municipality
...
# calculating the buldings per area ratio, combining the results in one DataFrame and sorting them 
...
# selecting the buildings in Tampere
tampere_buildingsDF= task2DF.filter(task2DF.municipality == "Tampere").dropDuplicates(["building_id"]) 

# calculating the average latitude and longitude for the buildings in Tampere
...
# selecting the buildings in Hervanta
hervanta_buildingsDF= tampere_buildingsDF.filter(task2DF.postal_code == "33720")

# calculating the average latitude and longitude for the buildings in Hervanta
...
# calculating the distances between the buildings and the average location
...
# selecting the first building and the first distance in the sorted list and formulating the result
